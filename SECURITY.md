...
Production serves the frontend over HTTPS so the initial delivery of the application bundle and its pinned backend public key is protected by the browser TLS trust model before the application-layer secure transport starts.

The local certificate generated by `scripts/generator.py frontend-certs` is self-signed and suitable for local testing only. Public or shared deployments must use a certificate trusted by clients; otherwise users can still be trained to click through certificate warnings, which weakens first-load security.
