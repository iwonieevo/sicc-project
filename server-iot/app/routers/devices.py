...
def get_device_queue(device_id: int, limit: int = 50, db: Session = Depends(get_db)):
    ...
    query = (
        ...
        .filter(VCommandLog.device_id == device_id)
...
def cancel_queue_task(device_id: int, queue_id: int, db: Session = Depends(get_db)):
    """Cancel queued command if it has not been started yet"""

    queue_log = (
        db.query(VCommandLog)
        .filter(VCommandLog.queue_id == queue_id, VCommandLog.device_id == device_id)
        .first()
    )
    ...
    return QueueCancelResponse(status="cancelled", device_id=device_id, queue_id=queue_id)
