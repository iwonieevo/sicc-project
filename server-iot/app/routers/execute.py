...
from app.schemas import CommandStatusResponse, ExecuteRequest, ExecuteResponse
...
async def execute_command(request: ExecuteRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        param_defs = (
            ...
            .filter(CommandParameter.command_id == request.command_id, CommandParameter.is_deleted == False)
            .all()
        )
        ...
        for param_def in param_defs_list:
            if (
                ...
                raise HTTPException(
                    status_code=400, detail=f"Missing required parameter: {param_def['name']}"
                )

        sanitized_params = sanitize_parameters(request.parameters, param_defs_list)

        queue = CommandQueue(
            device_id=request.device_id, command_id=request.command_id, parameters=sanitized_params
        )
        db.add(queue)
        db.commit()
        db.refresh(queue)
        ...
        LOGGER.info(
            f"Command queued: device={request.device_id}, command={request.command_id}, queue_id={queue.id}"
        )
...
@router.get("/logs")
def get_execution_logs(limit: int = 50, db: Session = Depends(get_db)):
